...
var yass = {
    ...
    incorrect_body_height_issue: function() {
        ...
        if (this.m_sobj && this.m_sobj.offset != 0)
            return;
    ...
    getbedgesize: function() {
        return Math.min(this.m_sobj.bodyClientHeight() * 0.25, this.m_edgesize);
    },
    ...
    {
        if (!this.m_pref.enabled)
            return;
        if (ev.altKey || ev.ctrlKey || (ev.keyCode != 32 && ev.shiftKey))
            return;
        ...
        switch (ev.type)
        {
            ...
            case "mousedown":
                ...
                if (this.m_sobj && this.m_sobj.offset / this.m_lastd < 0)
                    return;
        ...
        ev.preventDefault();
        if (ev_detail == 0)
            return; // maybe this fixes microsoft smooth wheel problem

        var ctm = (new Date()).getTime();

        var pixelstep = 0;
        if (!fromkeyboard) {
            pixelstep = ev_detail * (this.m_pref.wheelstep / 120.0);
        }

        // ignore events in edge bounce animation
        // for MacOS native flick
        if (pixelstep != 0 && this.m_edgesize != 0) {
            if (ctm - this.m_lasteventtime <= 200) {
                if (((this.m_sobj.offset < 0 || this.m_vpos < 5) && pixelstep < 0) ||
                        ((this.m_sobj.offset > 0 || this.m_vpos > this.m_sobj.maxscroll - 5) && pixelstep > 0)) {
        ...
        var evtime = (ctm - this.m_lasteventtime);
        var speed = Math.min(300, evtime * (120.0 / Math.abs(ev_detail))); // 120 = 1 tick of wheel
        this.m_speed = (this.m_speed + speed) * 0.5;
        //yass.dump("e detail:" + ev_detail + " time:" + (ctm - this.m_lasteventtime) +  " speed:" + this.m_speed);
        ...
        if (!this.m_wheelactive)
        {
            ...
            var last_scrolltop = this.m_sobj.body.scrollTop;
            ...
            this.m_lastfunctime = (new Date()).getTime();
            -17;
        ...
        else
        {
            ...
            if (fromkeyboard) {
                if (!pagescroll)
                    accel = this.m_pref.kbdaccel / 100;
            } else {
        ...
        F.prototype = {
            ...
            adjust: function(newp, oldp) {
                ...
                if ((this.body.scrollTop == 0 && moveto < 0)
                        ...
                        || ((this.maxscroll - this.body.scrollTop) <= 1 && moveto > 0))
                    return false;
                return true;
            },
        ...
        F.prototype = {
            ...
            adjust: function(newp, oldp) {
                ...
                if (scrollsize != this.maxscroll) {
                    ...
                    if (this._lastscrollheight == this.body.scrollHeight && newp > this.maxscroll)
                        return false; // bot edge's height change changes maxscroll - emergency stop
                    this.maxscroll = scrollsize;
                }
            ...
            stop: function() {
                ...
                if (this.body.scrollTop > this.maxscroll)
                    this.body.scrollTop = this.maxscroll;
                this.offset = 0;
            },
            release: function() {
                this.stop();
                if (this.body._yass_ownedby == this)
                    this.body._yass_ownedby = null;
            },
            ...
            render: function(offsetscroll, pos) {
                ...
                {
                    if (this.ebot.e.clientHeight < offsetscroll)
                        this.ebot.e.style.height = (offsetscroll + 50) + "px";
    ...
    edgetop: {
        ...
        setowner: function(owner) {
            ...
            if (this._owner == owner || this.e == owner)
            ...
            if (this._owner) {
                try {
                    ...
                    if (this._ancientnode)
                        this._ancientnode.style.marginTop = this._ancientnode_marginback; // restore offset - must be tested
                } catch (ex) {
                }
            }
            ...
            if (owner == owner.ownerDocument.body) {
                ...
                if (this._ancientnode) {
                    ...
                    if (astyle.getPropertyCSSValue("display").cssText == "none")
                        this._ancientnode = null;
                    //if(astyle.getPropertyCSSValue("margin-top").getFloatValue(5) != 0) this._ancientnode = null;
                    if (this._ancientnode.style.marginTop != "")
                        this._ancientnode = null;
                }
                if (this._ancientnode) {
            ...
            if (bodypaddingtop > 0)
            {
                if (owner.childNodes.length == 0)
                    owner.appendChild(this.epadding);
                else
                    owner.insertBefore(this.epadding, owner.childNodes[0]);
            ...
            if (owner.childNodes.length == 0)
                owner.appendChild(this.e); // edge is below the dummy element
            else
                owner.insertBefore(this.e, owner.childNodes[0]);
            if (owner.childNodes.length == 0)
                owner.appendChild(this.dummy); // dummy is above
            else
                owner.insertBefore(this.dummy, owner.childNodes[0]);
            return 0;
        },
        ...
        render_abs: function(offset) {
            for (var i = 0; i < this.abselms.length; i++) {
    ...
    funcwheel: function(kbd, idelta, edgemargin)
    {
        ...
        if (this.m_vpos < 0)
        {
            ...
            if (!this.m_frameelement.topframe)
    ...
    {
        if (ev.target == this.m_lastcheckdesignmodearg)
            return;
        ...
        if (mode && mode.toLowerCase() == "on")
            b = false;
        if (ev.target.isContentEditable == true)
            b = false;
        if (ev.target.style["-webkit-user-modify"] != "")
            b = false;
    ...
    findNodeToScroll: function(orig, hint, log)
    {
        ...
        function getscrolltype(wscroll, wclient, hscroll, hclient)
        {
            if (hclient < 50)
                return 0;
            if (hscroll - hclient < 10)
                hclient = hscroll; // too small to scroll
        ...
        var newobject = function(p) {
            var editable = false;
            if (p.nodeName.toLowerCase() == "textarea")
                editable = true;
            ...
            if (p.isContentEditable)
                editable = true;
            if (p.style["-webkit-user-modify"] != "")
                editable = true;

            if (editable)
                return yass.scroller_noedge;
        ...
        {
            if (node != body)
                return node.clientHeight + getstyle(node, "border-top-width") + getstyle(node, "border-bottom-width");
        ...
        do {
            ...
            {
                var res = {"a": "1", "blacklist": "", "dolog": "", "edgetype": "0", "enabled": "1", "preset_green": "40,764,224,386,86,491,227,105", "selectedpreset": "green", "usepixelscroll": "false"};
...
//		this.m_pref.showcontextmenu = ps.GetBoolPref("extensions.yass.showcontextmenu");
            ...
            request_listener: function(req, send, res) {
                if (req.msg == "iframe_push") {
                    if (document == null)
                        return;
                    if (document.URL.indexOf(req.src) != 0)
                        return;
...
// run immediately
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            for (i in iframes) {
                framearray.push(iframes[i]);
            }
            for (i in frames) {
                framearray.push(frames[i]);
            }
            ...
            var res = {"msg": "iframe_push", "src": "https://www.google.com/blank.html", "scrolling": "null", "topframe": false};
