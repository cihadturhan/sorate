...
function print_header() {
    ...
    foreach ($data as $key => $value) {
        if ($key != 0) {
			...
			$title = iconv('UTF-8',  "UTF-8//IGNORE", substr($value['groupName'], 0, 20));
            $newSheet->setTitle('0.'.$title);
    ...
    foreach ($data as $key => $value) {
        ...
        if ($worksheet){
			$title = iconv('UTF-8',  "UTF-8//IGNORE", substr($value['groupName'], 0, 20));
            $worksheet->setTitle(($key + 1) . '.' . $title);
		}

        $excel->setActiveSheetIndex($key);

        $worksheet->setCellValueByColumnAndRow(2, 6, $value['queryDate'][0])
                ->setCellValueByColumnAndRow(6, 6, $value['queryDate'][1])
                ->setCellValueByColumnAndRow(2, 8, $value['groupName']);
