...
function TSort_StoreDef () {
    ...
    this.history = [];
    this.sort_keys = [];
    this.sort_colors = [ '#DB2A81', '#00B2F0', '#3C5492' ];
};
...
function tsSort(a, b)
{
    ...
    for (var i = 0; i < len; i++)
    {
        ...
        if	((type == 'i')||(type == 'f')||(type == 'd'))
            ...
            size_a = v_a.substr(-2);
            size_b = v_b.substr(-2);
            if(sizeArr[size_a]!=sizeArr[size_b]){
                result = sizeArr[size_a]-sizeArr[size_b];
            }else{
                int_a = parseFloat(v_a.substr(0, v_a.length-3));
                if (isNaN(int_a)) int_a = 0;
                int_b = parseFloat(v_b.substr(0, v_b.length-3));
                if (isNaN(int_b)) int_b = 0;
                result = (int_a*sizeArr[size_a] > int_b*sizeArr[size_b])? 1:-1
            }
        }else
            result = (v_a < v_b)? -1: 1;
